...
try:
    if os.name == 'nt' and not sys.stdout.isatty():
        # Крокозябры в 'python ... | Out-File': PowerShell декодирует пайп через
        # [Console]::OutputEncoding (OEM cp866 на русской Windows), а Python по
        # умолчанию пишет UTF-8. Дочерний процесс не может поменять декодер
        # родителя, поэтому пишем в CP консоли (GetConsoleOutputCP), а не в UTF-8.
        # Colab/Linux и прямой консольный вывод остаются UTF-8.
        import ctypes
        _cp = ctypes.windll.kernel32.GetConsoleOutputCP()
        _enc = 'utf-8' if _cp in (0, 65001) else f'cp{_cp}'
...
class _Tee:
    """Дублирует весь stdout в буфер — полный вывод анализа уходит в HTML."""
...
pre.full{background:#161b22;border:1px solid #30363d;border-radius:8px;padding:10px 12px;
white-space:pre-wrap;word-break:break-word;font-size:12px;line-height:1.35;color:#c9d1d9}''')
    ch.append('</style></head><body>')
    ...
    if full_text:
        ch.append('<h2>FULL OUTPUT — полный вывод анализа</h2>')
        ch.append('<pre class="full">' + H.escape(full_text) + '</pre>')
