...
- Шаг Адама ≈ lr → лог-частоты требуют ~3/lr шагов: **lr 6e-4, warmup 300** (было 3e-4/1200).
  Результат: val 8.49@7315 (uniform 11.09, униграмма 7.48).
...
| Цель | Статус |
|---|---|
| val < 11.09 (uniform) | ✅ Достигнуто (8.49@7315) |
| val < 9.0 | ✅ Достигнуто (8.49@7315; старый прогон — 8.77@15844) |
| val < 8.5 | ✅ Достигнуто (8.4877@7315) |
...
| Открытый словарь (лакуна/фантомы) | ✅ M52b/M54/M59 (диагностика; на однородных данных фантомы спят) |
...
|---|---|---|---|
| best.pt (текущий) | 7315 | **8.4877** | M50–M61 + lr 6e-4/warmup 300, depth 12/24 |
| best.pt (прошлый прогон) | 250 | 11.1584 | M52–M56, lr 3e-4/warmup 1200 |
| best.pt (локальный) | 1045 | 13.3991 | старый прогон (до взрыва 2970) |
...
- **Корпус — настоящая русская проза** (жанровые потоки WideBind\wb): униграмма 7.48 нат,
  биграмма 4.21, uniform 11.09. Модель на 8.49 — учит частоты; до униграммы ~1 нат.
- **Спайки детерминированные**: один и тот же батч (шаг 165 и т.д.) валит голову в насыщение
  (`sat=1.0`) при любом lr ≥ ~3e-4. Стенка (M52a) держит градиент живым → восстановление
  за 1–2 логинтервала. Корень не bus_bias (M61-кап не помог) — подозреваются `z/T` (рост readout)
  или λ-акцент; вскрывается analyze по чекпойнту сразу после спайка.
- **Униграмма учится арифметикой Адама**: шаг ≈ lr, лог-частоты (−3…−5) требуют ~10k шагов
  при 3e-4. M60 (6e-4 + warmup 300) сократил это вдвое и дал наблюдаемый спад.
- **«Таймеры» вместо измерений** (цепочка M57): `mat` (pen_init=1.0 → readiness = decay от 1.0,
  ...
  от resvar. Исправлено в M58 (`pen_init=0`).
- **Маркеры M47 (`mlp_out` 1400→424, `diversity` 0.6→0.08) неспецифичны** — это функция весов,
  а не качества: в текущем прогоне они стоят на ~350–500/0.07–0.16 при val 8.49 (лучшем за историю).
- **DepthController открыл 12/24 на 8360** (val-плато slope≈0) — начинается второй этап.

## TODO

- [ ] Найти корень спайков (analyze по чекпойнту сразу после спайка: `readout std`, `emphasis_gain`,
      `log_temp`, `bus_head_proj`) — если частота вырастет, спайки съедят бюджет.
- [ ] Дойти до униграммы (7.48) и ниже; далее биграммная структура (4.21).
- [ ] Следить за памятью при открытии 24/24 (сейчас 32.7/28.9 при 12/24; порог 40GB).
- [ ] M55-рост: подтверждённые фантомы → новые биты (на однородных данных фантомы спят,
      `ph_births` растут медленно — ждём новизны/OOD).
- [ ] UCL: `read_scale` с floor до 5k; после — проверить, закроет ли модель сама (если да —
      UCL бесполезен на этих данных, концепты живут через голову A/B/C).
