...
- **Step:** ~8 470+ (Colab, прогон идёт со вчерашнего вечера — первый прогон, выживший ночь)
- **Val:** **8.4877@7315 — ТЕКУЩИЙ BEST** (этот прогон); 8.53@8360 (плато → DepthController открыл 12/24)
- **Val-траектория (этот прогон):** 18.31@250 (повреждён спайком) → 13.35@500 → 10.80@750 →
  10.47@1750 → 10.29@2500 → 9.92@2750 → 9.78@3135 → 9.51@4180 → 9.05@5225 → 8.71@6270 → **8.49@7315**
- **Контекст корпуса (измерено, WideBind\wb):** uniform = 11.09 нат, **униграмма = 7.48**, биграмма = 4.21.
  Модель прошла от uniform до 8.49 — **выучена частотная структура**, приближается к униграмме.
- **Расписание (M60):** lr **6e-4**, warmup **300** (было 3e-4/1200) — это и дало ускоренный спад.
- **Спайки:** детерминированные (165, 3520, 3795, 4895, 5665, 6270, 6985, 7480, 7755, 8305):
  `sat=1.0`, `head_wall` 1000–3400 — стенка/AGC/plateau-damping каждый раз восстанавливают,
  тренд не теряется (цена ~50–100 шагов). Корень ещё не найден (не bus_bias — M61-кап не помог).
- **DepthController:** на 8360 открыл 12/24 (val-плато); память 32.7/28.9 GB.
- **NaN:** 0, Inf: 0.
...
- **Step:** ~8 470+ (Colab, идёт с вечера; первый прогон, выживший ночь)
- **Val:** 8.4877@7315 (best), 8.53@8360 → depth 12/24
- **Memory Bank:** активен, пишет 1× за форвард (M58); maturation-gated
- **Bridge:** stable, bridge_conn ~6.14 (по aux-шкале), lbg_tau ~0.97
- **Maturation:** deep-first; `pen_init=0` (M58) — readiness теперь от компетентности, не таймер
- **Голова:** M52–M61 — стенка, λ, лакуна/фантомы, SRL (диагностика), tempering, bus-cap
- **Концепт-системы:** UCL ↔ фантом-банк связаны (M59 A/B/C), floor UCL до 5k
- **Известное открытое:** корень спайков (не bus_bias) и UCL-градиент записи (~1e-5 живой, но медленный)
