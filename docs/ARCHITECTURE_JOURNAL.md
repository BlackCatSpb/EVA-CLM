...
- **Прогон:** свежий, git `4725d59` (T9-комплект тренировки; T9.11-генерация — отдельный скрипт), seq 384, batch 1, A100 40GB, план 150k шагов.
- **Step:** ~4 400+ (2026-09-20); **best val 9.1452@3960**; 17 улучшений из 21 eval.
- **Val-траектория:** 15.78@250 → 12.02@440 → 10.39@1000 → 10.10@1760 → 9.98@2200 →
  9.71@2750 → 9.58@3080 → 9.32@3520 → **9.15@3960** → 9.43@4400. Спайки (500/750/2000/4400)
  совпадают с эпизодами насыщения головы — после каждого новый best.
- **Контекст корпуса (WideBind\wb):** uniform 11.09, **униграмма 7.48**, биграмма 4.21;
  тренировочный CE 7.34 → 4.4–4.8.
...
- **Фантом:** наблюдений пока нет (`ph_obs=0`): порог 1.1 выше хвоста `sal_p99≈1.03`;
  хвост медленно растёт (1.016@1045 → 1.036@4345).
- **Банк памяти:** пишет с ~495 — гейт зрелости слоёв перешёл порог 0.3
  (профиль зрелости 0.18→0.88 по слоям, глубокие открываются).
- **Голова:** два крупных спайка насыщения (ce 201@495, 90@1430) — safety-стена видна в aux
  (55.1/205.1) и гасит за ~55 шагов; на OOD-пробе голова сатурируется (∂CE/∂h 1e-9…1e-4,
  L23 доминирует), `CARRY≈0` — потоковую непрерывность несёт кэш, не VSA.
...
- Шаг Адама ≈ lr → лог-частоты требуют ~3/lr шагов: **lr 6e-4, warmup 300** (было 3e-4/1200).
  Результат (текущий прогон): CE 7.34 → 4.4–4.8, val 9.1452@3960 (uniform 11.09, униграмма 7.48).
...
| val < 9.0 | ⏳ 9.1452@3960 (лучший) |
| val < 8.5 | ⏳ Следующая после 9.0 |
...
|---|---|---|---|
| best.pt (текущий) | 3960 | **9.1452** | прогон 4725d59 (T9-комплект), depth 8/24 |
| `best 4840.pt` (локальная копия) | 3960 | 9.1452 | тот же чекпойнт: оценка 4840 улучшения не дала |
...
- **Step:** ~4 400+ (свежий прогон git `4725d59`, 2026-09-20); **best val 9.1452@3960**
- **Val-траектория:** 15.78@250 → 12.02@440 → 10.39@1000 → 10.10@1760 → 9.98@2200 →
  9.71@2750 → 9.58@3080 → 9.32@3520 → **9.15@3960** → 9.43@4400
...
- **UCL:** чтение открыто полом (0.116), 3/8 слотов; **Фантом:** наблюдений нет (порог выше хвоста)
- **Банк:** пишет с ~495 — гейт зрелости слоёв перешёл порог 0.3; **Генерация:** smart-режим с паритетом тренировки
- **Известное открытое:** порог наблюдения фантома (sal_p99≈1.03 vs thr 1.1); рост intent-шины;
  бюджет tok/s 62–65 (~54k шагов за 94ч)
