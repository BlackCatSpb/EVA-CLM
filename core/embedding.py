...
class PartitionedEmbedding(nn.Module):
    ...
    Математические свойства:
      - rank(E) = K (НЕ полный): все 65 536 токенов живут в K-мерном
        подпространстве span{readout}; измерено σ_K/σ_0 ≈ 7e-7. Это замысел
        (голова читает то же подпространство тем же базисом — tying), а не
        дефект: roundtrip код→состояние→код точен на инициализации.
