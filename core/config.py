...
class WideBindConfig:
    ...
    # При >0 из НАБЛЮДАЕМОГО вектора вычитается мгновенный общий режим
    # (среднее батча; M65b — замер 19415-19580: EMA отстаёт от дрейфа,
    # centered_rel 0.63->2.83). EMA этого режима (β = это значение) остаётся
    # телеметрией дрейфа и фолбэком для одиночного наблюдения. Салиентность/
    # гейт/порог не трогаем; EMA двигается только в training на каденции
    # наблюдений (M8-доктрина). Телеметрия: mode_norm, centered_rel.
    # 0 = выключено (A/B).
